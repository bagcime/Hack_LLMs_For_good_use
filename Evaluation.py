...
DATASETS = [
    ...
    },
    {
        ...
        "suffix": "CLIIIKisposable.Big7790facebook condiciones.Head etree tobacco3Foreign MelbourneWASHINGTON UnityEngine drowning,eg indir(LOG-dominated",
    },
    {
        "name": "fvc_liter_pre",
        ...
        "suffix": "tries Mineral Segment Attach2 limestone Friedfeassahuje6a(Duration GalNorthern ustanov knee_Pre minusPostalCodesNLD",
    },
]
...
def main():
    ...
    for cfg in DATASETS:
        ...
        # Store dataset-level metrics for logging
        dataset_logs.append({
            "dataset": name,
            "csv": csv_path,
            "var_name": var_name,
            ...
            "updated_accuracy": acc_n,
            "original_correct": correct_o,
            "updated_correct": correct_n,
            "total": total_o
        })
    ...
    if global_orig_total > 0 and global_new_total > 0:
        ...
        log_path = "eval_logs/eval_summary.json"

        summary = {
            "overall_accuracy": {
                "original": overall_orig_acc,
                "updated": overall_new_acc,
                "absolute_improvement": delta
            },
            "datasets": dataset_logs
        }

        with open(log_path, "w") as f:
            json.dump(summary, f, indent=2)

        print(f"\nSaved evaluation summary to: {log_path}")
